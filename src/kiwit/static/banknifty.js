...
(() => {
  ...
  async function sync() {
    ...
    try {
      ...
      for (const [id, value] of Object.entries({
        ...
        'bn-trade-stop': s?.trade_stop_pct ?? s?.loss_pct,
        'bn-trade-target': s?.trade_target_pct ?? s?.profit_pct
      })) {
        const input = el(id);
        if (input) { input.disabled = usedToday; if (usedToday && value != null) input.value = value; }
      }
      const capControl = el('bn-profit-cap-enabled');
      if (capControl) {
        capControl.disabled = usedToday;
        if (usedToday) capControl.checked = s.session_profit_cap_enabled !== false;
      }
  ...
  async function action(event, name) {
    ...
    try {
      const body = name === 'run' ? JSON.stringify({amount: el('bn-amount').value, loss_pct: el('bn-loss').value, profit_pct: el('bn-profit').value, session_profit_cap_enabled: el('bn-profit-cap-enabled')?.checked ?? true, trade_stop_pct: el('bn-trade-stop')?.value || el('bn-loss').value, trade_target_pct: el('bn-trade-target')?.value || el('bn-profit').value}) : undefined;
